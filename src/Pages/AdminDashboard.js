...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await axios.get('http://localhost:8083/roles'); // Adjust endpoint as needed
  ...
  const handleDelete = async (userId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://localhost:8083/delete/${userId}`);
  ...
  const handleUserUpdate = async (event) => {
    ...
    try {
      await axios.post(`http://localhost:8083/edit/${selectedUser.id}`, selectedUser);
